...
void GLogger::_bind_methods() {
    BIND_STATIC_METHOD(GLogger, log, "msg", "msgColor");
}
...
void GLogger::log(const String &msg, const Color &msgColor) {
    ...
    UtilityFunctions::print_rich(
        "[color=" + metaColorHex + "]" + "[" + timeStr + "]" + "[LOG]" +
        "[/color][color=" + msgColorHex + "] " + msg + "[/color]");

    const std::string bufMsg = std::format("[{}] {}", timeStr.ascii().get_data(), msg.ascii().get_data());
    logBuffer.push_back((logMsg){bufMsg, msgColor});
}

const std::vector<GLogger::logMsg>& GLogger::getLogBuffer() {
    return logBuffer;
}

void GLogger::clearLogBuffer() {
    logBuffer.clear();
}
